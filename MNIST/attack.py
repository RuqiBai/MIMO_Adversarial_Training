    ...
    def step(self, original_inputs, inputs: torch.Tensor, targets: torch.Tensor, grad: torch.Tensor, f=False):
        with torch.no_grad():
            for i in range(self.ensembles):
                inputs[:, i:(i + 1), :, :] = inputs[:, i:(i + 1), :, :] + self.alpha[
                    i] * self._deepest_grad(grad[:, i:(i + 1), :, :], self.norm[i])
                delta = inputs[:, i:(i + 1), :, :] - original_inputs[:, i:(i + 1), :, :]
                delta = self._project(delta, self.norm[i], self.epsilon[i])
                inputs[:, i:(i + 1), :, :] = torch.clamp(inputs[:, i:(i + 1), :, :] + delta, min=0, max=1)
        return inputs
        ...
        with torch.no_grad():
            ...
            for i in range(len(self.epsilon)):
                ...
                for j in range(self.ensembles):
                    unit = self._deepest_grad(grad[:, j:(j + 1), :, :], self.norm[i])
                    # print(self.norm[i])
                    # print(torch.norm(unit.reshape(unit.shape[0],-1), dim=1, p=self.norm[i]))
                    inputs_tmp[:, j:(j + 1), :, :] = inputs_tmp[:, j:(j + 1), :, :] + self.alpha[i] * unit
                    delta = inputs_tmp[:, j:(j + 1), :, :] - original_inputs[:, j:(j + 1), :, :]
                    delta = self._project(delta, self.norm[i], self.epsilon[i])
                    if f:
                        f.write(str(torch.norm(delta.reshape(delta.shape[0], -1), dim=1, p=self.norm[i])[0].item()))
                        f.write(',')
                    inputs_tmp[:, j:(j + 1), :, :] = torch.clamp(original_inputs[:, j:(j + 1), :, :] + delta, min=0, max=1)
            ...
            for i in range(self.ensembles):
                inputs_update.append(inputs_step[max_index[i]][:, i:(i+1), :, :])
    ...
    def generate(self, inputs, targets):
        ...
        def _random_start(inputs):
            ...
            for i in range(len(self.norm)):
                adv_inputs[:, i:i + 1, :, :] = self._random_project(inputs[:, i:i + 1, :, :], self.norm[i],
                                                                    self.epsilon[i])
