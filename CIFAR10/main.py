...
parser.add_argument('--epochs', default=50, type=int, help='number of epochs to train [default: 200]')
parser.add_argument('--norm', nargs='+', type=float)
parser.add_argument('--max_iter', default=50, type=int)
parser.add_argument('--max_lr', default=1e-01, type=float)
...
net = ResNet18(3*args.ensembles,30).to(device)
    ...
    def test(epoch, net):
        ...
        if epoch > 0:
            ...
            if not os.path.isdir('checkpoint'):
                os.mkdir('checkpoint')
            torch.save(state, './checkpoint/ckpt_mimo_mnist_l3_03_{}.pth'.format(epoch))
            best_acc = acc
