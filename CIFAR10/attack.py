...
class AttackStep(object):
    def __init__(self, alpha, epsilon, norm, ensembles):
        self.alpha = alpha
...
class PGDStep(AttackStep):
    def __init__(self, model: ModelWrapper, alpha: list, epsilon: list, norm: list, ensembles: int):
        super().__init__(alpha, epsilon, norm, ensembles)
        self.model = model
    ...
    def step(self, original_inputs, inputs: torch.Tensor, targets: torch.Tensor, grad: torch.Tensor, f=False):
        with torch.no_grad():
            for i in range(self.ensembles):
                inputs[:, 3*i:3*(i + 1), :, :] = inputs[:, 3*i:3*(i + 1), :, :] + self.alpha[
                    i] * self._deepest_grad(grad[:, 3*i:3*(i + 1), :, :], self.norm[i])
                delta = inputs[:, 3*i:3*(i + 1), :, :] - original_inputs[:, 3*i:3*(i + 1), :, :]
                delta = self._project(delta, self.norm[i], self.epsilon[i])
                inputs[:, 3*i:3*(i + 1), :, :] = torch.clamp(inputs[:, 3*i:3*(i + 1), :, :] + delta, min=0, max=1)
        return inputs
...
class MSDStep(AttackStep):
    ...
    def calculate_loss(self, x, ground_truth):
        with torch.no_grad():
            loss = torch.zeros(self.ensembles, dtype=torch.float)
            y = self.model(x)
            for i in range(self.ensembles):
                loss[i] = self.model.criterion(y[:, i * self.model.num_classes:(i + 1) * self.model.num_classes], ground_truth[:, i])
            return loss
    ...
    def step(self, original_inputs, inputs: torch.Tensor, targets: torch.Tensor, grad: torch.Tensor, f=False):
        ...
        with torch.no_grad():
            ...
            for i in range(len(self.epsilon)):
                ...
                for j in range(self.ensembles):
                    unit = self._deepest_grad(grad[:, 3*j:3*(j + 1), :, :], self.norm[i])
                    # print(self.norm[i])
                    # print(torch.norm(unit.reshape(unit.shape[0],-1), dim=1, p=self.norm[i]))
                    inputs_tmp[:, 3*j:3*(j + 1), :, :] = inputs_tmp[:, 3*j:3*(j + 1), :, :] + self.alpha[i] * unit
                    delta = inputs_tmp[:, 3*j:3*(j + 1), :, :] - original_inputs[:, 3*j:3*(j + 1), :, :]
                    delta = self._project(delta, self.norm[i], self.epsilon[i])
                    if f:
                        f.write(str(torch.norm(delta.reshape(delta.shape[0], -1), dim=1, p=self.norm[i])[0].item()))
                        f.write(',')
                    inputs_tmp[:, 3*j:3*(j + 1), :, :] = torch.clamp(original_inputs[:, 3*j:3*(j + 1), :, :] + delta, min=0, max=1)
                inputs_step.append(inputs_tmp)
                loss.append(self.calculate_loss(inputs_step[-1], targets))
            if f:
                f.write('\n')
                f.write('loss value: ')
            ...
            for i in range(self.ensembles):
                inputs_update.append(inputs_step[max_index[i]][:, 3*i:3*(i+1), :, :])
        return torch.cat(inputs_update, dim=1)


class PGDAttack(object):
    def __init__(self, model, alpha, epsilon, norm, ensembles, max_iteration, msd=False, random_start=True, verbose=True):
        self.model = model
        ...
        self.ensembles = ensembles
        if msd:
            self.attack = MSDStep(model, alpha, epsilon, norm, ensembles)
        else:
            self.attack = PGDStep(model, alpha, epsilon, norm, ensembles)
    ...
    def generate(self, inputs, targets):
        ...
        def _random_start(inputs):
            ...
            for i in range(len(self.norm)):
                adv_inputs[:, 3*i:3*(i + 1), :, :] = self._random_project(inputs[:, 3*i:3*(i + 1), :, :], self.norm[i],
                                                                    self.epsilon[i])
        ...
        for j in range(self.max_iteration):
            ...
            loss = 0
            for i in range(self.ensembles):
                loss += criterion(outputs[:, i * 10:(i + 1) * 10], targets[:, i])
            loss.backward()
